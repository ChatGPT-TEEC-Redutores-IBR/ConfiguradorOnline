...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function gerarLinkCompartilhamento(valorDigitado) {
        ...
        try {
            ...
            if (json.erro) {
                ...
                exibirAlerta("❌ Erro inesperado na busca.");
            }
        } catch (e) {
    ...
    document.querySelectorAll(".search-container input").forEach((input) => {
        input.addEventListener("keydown", (e) => {
            if (e.key === "Enter") {
                ...
                if (tratarBuscaProduto(input)) {
                    gerarLinkCompartilhamento(valorDigitado);
                }
            }
        });
    ...
    document.querySelectorAll(".search-container").forEach((container) => {
        ...
        container.querySelector(".search-icon")?.addEventListener("click", (e) => {
            const valorDigitado = input.value.trim();
            if (container.classList.contains("expanded") && valorDigitado !== "") {
                e.stopPropagation();
                if (tratarBuscaProduto(input)) {
                    gerarLinkCompartilhamento(valorDigitado);
                }
    ...
    const botaoLink = document.querySelector("#botaoGerarLink");
    const campoLink = document.querySelector("#campoCodigoReferencia");
    if (botaoLink && campoLink) {
        botaoLink.addEventListener("click", () => {
            const valor = campoLink.value.trim();
            if (valor) {
                gerarLinkCompartilhamento(valor);
            } else {
                exibirAlerta(
                    "❌ Preencha um código ou referência antes de gerar o link.",
                );
            }
        });
    }
    document.querySelectorAll("#formBusca").forEach((form) => {
        const campoBusca = form.querySelector("#termo");
        const botaoBusca = form.querySelector("#btnBuscarDesktopFiltro");
        if (!campoBusca) return;

        const acionarBusca = () => {
            const valor = campoBusca.value.trim();
            if (!valor) return;
            if (tratarBuscaProduto(campoBusca)) {
                gerarLinkCompartilhamento(valor);
            }
        };
...
if ("serviceWorker" in navigator && (location.protocol === "https:" || location.hostname === "localhost")) {
    window.addEventListener("load", () => {
        navigator.serviceWorker.register("/Service-Worker.min.js?versao=versao.3.0.0", { updateViaCache: 'none' })
