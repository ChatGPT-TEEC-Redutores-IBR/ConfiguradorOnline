...
var Swiper = (function () {
    ...
    function Resize(_ref) {
        ...
        on('init', () => {
            ...
            mainSwiper.on('slideChange transitionEnd resize', updateWidth); window.addEventListener('orientationchange', orientationChangeHandler);
        });
        on('destroy', () => {
            removeObserver();
    ...
    class A {
        ...
        moveIndexBy(t, i, s) {
            ...
            if (h.canLoop()) {
                ...
                e < 0 ? (e = 0) : e >= n && (e = n - 1), (t = e - h.potentialIndex);
...
!(function (t, i) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('.galeria-produto[data-folder]').forEach(function (container) {
            ...
            var video = container.dataset.video || '#';
  ...
  <div class="col-lg-5 col-md-10 col-12 fw-bold d-flex flex-column h-100 justify-content-between">\
    ...
    <div class="botoes__produto d-flex gap-4 flex-column align-items-stretch mb-3">\
      <a href="'+ site + '" class="btn btn-outline-orange btn-lg rounded-0 px-5 py-3 w-100"><strong>Site do Produto</strong></a>\
      <a href="'+ catalog + '" class="btn btn-outline-orange btn-lg rounded-0 px-5 py-3 w-100"><strong>Catálogo do Produto</strong></a>\
    </div>\
  </div>\
</div>';
            ...
            var addSlides = function (count) {
                ...
                for (var i = 0; i < count; i++) {
                    var src = '/Layout/Imagens/Produtos/' + folder + '/' + i + folder + '.webp';
                    var slide = document.createElement('div');
                    slide.className = 'swiper-slide';
                    var lazyAttr = i === 0 ? '' : ' loading="lazy"';
                    slide.innerHTML = '<figure class="elemento-figure"><a href="' + src + '">\
      <div class="img-block p-3">\
 <picture><img' + lazyAttr + ' src="' + src + '" alt="' + alt + '" class="img-fluid" /></picture>\
      </div></a></figure>';
                    const anchor = slide.querySelector('a');
                    const img = slide.querySelector('img');

                    // preload original image to get its natural dimensions
                    const preload = new Image();
                    preload.addEventListener('load', function () {
                        anchor.setAttribute('data-pswp-width', preload.naturalWidth);
                        anchor.setAttribute('data-pswp-height', preload.naturalHeight);
                    });
                    preload.src = anchor.href;

                    // fallback in case preload fails
                    if (img.complete) {
  ...
  <picture><img' + lazyAttr + ' src="'+ src + '" alt="' + alt + '" class="img-fluid w-100 h-100 object-fit-cover" /></picture>\  </div>';
                    thumbs.appendChild(thumb);
                }
            };

            fetch('/Paginas/Navegacao/ContarImagens.php?folder=' + encodeURIComponent(folder))
                .then(function (r) { return r.ok ? r.json() : null; })
                .then(function (d) { addSlides(d && d.count ? parseInt(d.count, 10) : 1); })
                .catch(function () { addSlides(1); })
                .finally(function () {
                    ...
                    var enableLoop = slidesCount >= maxSpv + 1;
                    if (slidesCount > maxSpv) {
                    ...
                    var thumbsSwiper = new Swiper(galeria.querySelector('.galeria__thumbs'), {
                        loop: enableLoop,
                        spaceBetween: 0,
                    ...
                    var mainSwiper = new Swiper(galeria.querySelector('.galeria__corpo'), {
                        loop: enableLoop,
                        spaceBetween: 10,
                        effect: 'fade',
                    ...
                    lightbox = new PhotoSwipeLightbox({
                        gallery: galeria.querySelector('.galeria__corpo'),
                        children: '.elemento-figure a',
                    ...
                    const updateWidth = function () {
                        const galleryHeight = parseFloat(getComputedStyle(corpoContainer).getPropertyValue('--gallery-height')) || 200;
                        const width = galleryHeight + 'px';
