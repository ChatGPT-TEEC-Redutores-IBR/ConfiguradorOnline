...
try {
    $pdo = new PDO("sqlsrv:server=$dbhost;Database=$db", $user, $password, [
      ...
      $stmtTot = $pdo->prepare(
        "SELECT COUNT(*) FROM _USR_CONF_SITE_HISTORICO_COTACAO WHERE DS_EMAIL = ?"
    );
    $stmtTot->execute([$email]);
    $total = (int)$stmtTot->fetchColumn();

    $stmt = $pdo->prepare(
        "SELECT DS_REFERENCIA AS produto, CD_OPORTUNIDADE AS oportunidade, DT_DATA AS data
           FROM _USR_CONF_SITE_HISTORICO_COTACAO
          WHERE DS_EMAIL = ?
    ...
    log_event('ListarCotacoes: ' . $e->getMessage());
    echo json_encode([]);
}
